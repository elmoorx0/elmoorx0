 ...
 *
 * Run: npx tsx --test packages/ai-dev/tests/ai-dev-smoke.test.mjs
 */
...
describe("ai-dev: smoke tests", () => {
  skip("package loads without throwing", () => {
    assert.ok(mod, skipReason || "package should load");
  });

  skip("expected exports are present", () => {
    const missing = ["generateTests", "detectBugs", "reviewCode", "analyzePerformance"].filter(name => !mod[name]);
  ...
  skip("exported functions are callable", () => {
    ...
    for (const name of ["generateTests", "detectBugs", "reviewCode", "analyzePerformance"]) {
      const v = mod[name];
  ...
  skip("AI dev tooling - basic shape", () => {
