 ...
 *
 * Run: npx tsx --test packages/ar-vr/tests/ar-vr-smoke.test.mjs
 */
...
describe("ar-vr: smoke tests", () => {
  skip("package loads without throwing", () => {
    assert.ok(mod, skipReason || "package should load");
  });

  skip("expected exports are present", () => {
    const missing = ["ARScene", "VRScene", "Model3D", "ARObject", "HandTracking", "SpatialAudio"].filter(name => !mod[name]);
  ...
  skip("exported functions are callable", () => {
    ...
    for (const name of ["ARScene", "VRScene", "Model3D", "ARObject", "HandTracking", "SpatialAudio"]) {
      const v = mod[name];
