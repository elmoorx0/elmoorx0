 ...
 *
 * Run: npx tsx --test packages/auto-test/tests/auto-test-smoke.test.mjs
 */
...
describe("auto-test: smoke tests", () => {
  skip("package loads without throwing", () => {
    assert.ok(mod, skipReason || "package should load");
  });

  skip("expected exports are present", () => {
    const missing = ["generateTests", "runGeneratedTests", "generateTestFile"].filter(name => !mod[name]);
  ...
  skip("exported functions are callable", () => {
    ...
    for (const name of ["generateTests", "runGeneratedTests", "generateTestFile"]) {
      const v = mod[name];
  ...
  skip("AI test generation - basic shape", () => {
