/**
 * @elmoorx/edge-functions — smoke tests
 *
 ...
 *
 * Run: npx tsx --test packages/edge-functions/tests/edge-functions-smoke.test.mjs
 */
...
describe("edge-functions: smoke tests", () => {
  skip("package loads without throwing", () => {
    assert.ok(mod, skipReason || "package should load");
  });

  skip("expected exports are present", () => {
    const missing = ["edgeFunctions", "edgeFunction", "getGeoFromRequest", "edgeCache", "edgeKV"].filter(name => !mod[name]);
  ...
  skip("exported functions are callable", () => {
    ...
    for (const name of ["edgeFunctions", "edgeFunction", "getGeoFromRequest", "edgeCache", "edgeKV"]) {
      const v = mod[name];
  ...
  skip("Edge functions - basic shape", () => {
