/**
 * @elmoorx/experiments — smoke tests
 *
 ...
 *
 * Run: npx tsx --test packages/experiments/tests/experiments-smoke.test.mjs
 */
...
describe("experiments: smoke tests", () => {
  ...
  skip("expected exports are present", () => {
    const missing = ["experiments", "experiment", "flags", "flag"].filter(name => !mod[name]);
  ...
  skip("exported functions are callable", () => {
    ...
    for (const name of ["experiments", "experiment", "flags", "flag"]) {
      const v = mod[name];
