/**
 * @elmoorx/marketplace — smoke tests
 *
 ...
 *
 * Run: npx tsx --test packages/marketplace/tests/marketplace-smoke.test.mjs
 */
...
describe("marketplace: smoke tests", () => {
  ...
  skip("expected exports are present", () => {
    const missing = ["catalog", "search", "install", "uninstall", "publish"].filter(name => !mod[name]);
  ...
  skip("exported functions are callable", () => {
    ...
    for (const name of ["catalog", "search", "install", "uninstall", "publish"]) {
      const v = mod[name];
  ...
  skip("Component marketplace - basic shape", () => {
