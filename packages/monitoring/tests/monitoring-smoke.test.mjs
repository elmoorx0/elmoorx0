/**
 * @elmoorx/monitoring — smoke tests
 *
 ...
 *
 * Run: npx tsx --test packages/monitoring/tests/monitoring-smoke.test.mjs
 */
...
describe("monitoring: smoke tests", () => {
  ...
  skip("expected exports are present", () => {
    const missing = ["MetricsRegistry", "MonitoringServer"].filter(name => !mod[name]);
  ...
  skip("exported functions are callable", () => {
    ...
    for (const name of ["MetricsRegistry", "MonitoringServer"]) {
      const v = mod[name];
  ...
  skip("Metrics and monitoring - basic shape", () => {
