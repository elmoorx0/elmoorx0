 ...
 *
 * Run: npx tsx --test packages/native/tests/native-smoke.test.mjs
 */
...
describe("native: smoke tests", () => {
  skip("package loads without throwing", () => {
    assert.ok(mod, skipReason || "package should load");
  });

  skip("expected exports are present", () => {
    const missing = ["View", "Text", "Button", "ScrollView", "StyleSheet", "useNavigation"].filter(name => !mod[name]);
  ...
  skip("exported functions are callable", () => {
    ...
    for (const name of ["View", "Text", "Button", "ScrollView", "StyleSheet", "useNavigation"]) {
      const v = mod[name];
  ...
  skip("Mobile native - basic shape", () => {
