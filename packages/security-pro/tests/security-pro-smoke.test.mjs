/**
 * @elmoorx/security-pro — smoke tests
 *
 ...
 *
 * Run: npx tsx --test packages/security-pro/tests/security-pro-smoke.test.mjs
 */
...
describe("security-pro: smoke tests", () => {
  skip("package loads without throwing", () => {
    assert.ok(mod, skipReason || "package should load");
  });

  skip("expected exports are present", () => {
    const missing = ["securityHeaders", "rateLimit", "Sanitizer", "CSRFProtection", "AuditLogger"].filter(name => !mod[name]);
  ...
  skip("exported functions are callable", () => {
    ...
    for (const name of ["securityHeaders", "rateLimit", "Sanitizer", "CSRFProtection", "AuditLogger"]) {
      const v = mod[name];
