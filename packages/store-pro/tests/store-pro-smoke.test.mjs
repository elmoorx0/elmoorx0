 ...
 *
 * Run: npx tsx --test packages/store-pro/tests/store-pro-smoke.test.mjs
 */
...
describe("store-pro: smoke tests", () => {
  skip("package loads without throwing", () => {
    assert.ok(mod, skipReason || "package should load");
  });

  skip("expected exports are present", () => {
    const missing = ["createStore", "select", "sync", "loggingMiddleware", "StoreDevtools"].filter(name => !mod[name]);
  ...
  skip("exported functions are callable", () => {
    ...
    for (const name of ["createStore", "select", "sync", "loggingMiddleware", "StoreDevtools"]) {
      const v = mod[name];
  ...
  skip("Advanced state management - basic shape", () => {
