 ...
 *
 * Run: npx tsx --test packages/templates/tests/templates-smoke.test.mjs
 */
...
describe("templates: smoke tests", () => {
  skip("package loads without throwing", () => {
    assert.ok(mod, skipReason || "package should load");
  });

  skip("expected exports are present", () => {
    const missing = ["blogTemplate", "dashboardTemplate", "ecommerceTemplate"].filter(name => !mod[name]);
  ...
  skip("exported functions are callable", () => {
    ...
    for (const name of ["blogTemplate", "dashboardTemplate", "ecommerceTemplate"]) {
      const v = mod[name];
  ...
  skip("Starter templates - basic shape", () => {
