 ...
 *
 * Run: npx tsx --test packages/testing/tests/testing-smoke.test.mjs
 */
...
describe("testing: smoke tests", () => {
  skip("package loads without throwing", () => {
    assert.ok(mod, skipReason || "package should load");
  });

  skip("expected exports are present", () => {
    const missing = ["render", "renderToString_", "fire", "flush", "expectSignal", "waitFor", "mockFetch"].filter(name => !mod[name]);
  ...
  skip("exported functions are callable", () => {
    ...
    for (const name of ["render", "renderToString_", "fire", "flush", "expectSignal", "waitFor", "mockFetch"]) {
      const v = mod[name];
  ...
  skip("Testing utilities - basic shape", () => {
